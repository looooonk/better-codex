...
This repository is a fork of the open source Codex CLI codebase, but its goal is
not to stay a drop-in clone of upstream. The project is being reshaped into a
standalone terminal application built on top of Codex's Rust backend.

When making changes in this fork:

- Treat the existing Codex CLI as backend infrastructure and reference material,
  not as the target user experience. However, make sure to never modify Codex's
  agent harness.
- Optimize new work for a full-screen, app-like TUI in the spirit of OpenCode,
  btop, and other dense terminal applications.
- Prefer new UI architecture over incremental patches to the upstream chat UI
  when the desired behavior diverges materially.
- Keep the backend boundaries clean: isolate reusable agent/session/execution
  logic from presentation code, and avoid adding UI concerns to backend crates.
- Avoid preserving upstream release, CI, publishing, issue triage, and
  repository-maintenance machinery unless it directly supports local development
  of this fork.
- Keep dependencies and tooling pragmatic. Add only what is needed for the new
  TUI or the retained Codex backend, and remove inherited infrastructure when it
  becomes noise.
- README and top-level project docs should describe this fork's direction, not
  upstream Codex distribution channels.
