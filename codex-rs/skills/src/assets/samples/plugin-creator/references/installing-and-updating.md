...
- the user wants Better Codex to see the updated plugin without manually editing marketplace files
...
outside Better Codex depends on a specific token:
...
```bash
better-codex plugin add <plugin-name>@<marketplace-name-from-marketplace-json>
```

The default personal marketplace is discovered implicitly from
`~/.agents/plugins/marketplace.json`. You do not need `better-codex plugin marketplace add` for that
path, and `better-codex plugin marketplace list` is not the right check for whether that default
marketplace exists.
...
```bash
better-codex plugin add <plugin-name>@<local-marketplace>
```

6. Prompt the user to use a new thread to try the updated plugin, so that Better Codex picks up new
   skills and tools.
...
- Read the personal marketplace name with
  ...
  `better-codex plugin add <plugin-name>@<marketplace-name>`.
...
- Do not tell the user to run `better-codex plugin marketplace add` for the default
  personal-marketplace flow. That marketplace is discovered implicitly by Better Codex.
...
- Use `better-codex plugin list` when the plugin lives in a different configured marketplace and you
  need to confirm which marketplace is surfacing that plugin.
- If a non-default local marketplace has not been configured yet, install it with
  `better-codex plugin marketplace add <path-to-marketplace-root>` before telling the user to run
  `better-codex plugin add <plugin-name>@<marketplace-name>`.
