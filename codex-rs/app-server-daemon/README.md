...
`codex-app-server-daemon` backs the machine-readable `better-codex app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for Better Codex instances launched over SSH, including fresh
developer machines that should expose app-server with `remote_control` enabled.
...
```sh
better-codex app-server daemon start
better-codex app-server daemon restart
better-codex app-server daemon enable-remote-control
better-codex app-server daemon disable-remote-control
better-codex app-server daemon stop
better-codex app-server daemon version
better-codex app-server daemon bootstrap --remote-control
```
...
The daemon assumes Better Codex is installed through the fork's `install.sh`
and always launches the standalone managed binary under
`BETTER_CODEX_INSTALL_ROOT`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `$BETTER_CODEX_INSTALL_ROOT/current/bin/codex` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `$BETTER_CODEX_INSTALL_ROOT/current/bin/codex` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `better-codex remote-control` bootstraps with `--remote-control` when the
