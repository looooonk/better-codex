...
fn agent_plugin_data_root_is_stable_and_unambiguous() {
    ...
    assert_eq!(
        first_root.as_path(),
        tmp.path().join(
            "plugins/data/agent-plugins/6920dd17774030852d11d1b94758fcaae4f894c7b2f36301ed174bc3b33e0743"
        )
    );
}
...
#[test]
fn agent_plugin_unsafe_version_uses_stable_cache_key() {
    let tmp = tempdir().unwrap();
    let plugin_root = tmp.path().join("agent-plugin");
    fs::create_dir_all(&plugin_root).unwrap();
    fs::write(
        plugin_root.join("plugin.json"),
        r#"{"$schema":"https://agent-plugins.org/schemas/1.0.0/plugin.schema.json","name":"agent-plugin","version":"release/2026"}"#,
    )
    .unwrap();
    let plugin_id = PluginId::new("agent-plugin".to_string(), "debug".to_string()).unwrap();
    ...
    assert!(result.plugin_version.starts_with("agent-plugins-"));
    assert_eq!(result.plugin_version.len(), "agent-plugins-".len() + 24);
