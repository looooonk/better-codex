...
pub(crate) async fn run_inline_auto_compact_task(
    ...
    let (step_context, world_state, preserved_turn_items) = match initial_context_injection {
        InitialContextInjection::BeforeLastUserMessage(world_state) => {
            (step_context, world_state, Vec::new())
        }
        InitialContextInjection::AfterSummary {
            step_context,
            world_state,
            turn_items,
        } => (step_context, world_state, turn_items),
...
async fn run_compact_task_inner(
    ...
    sess.emit_turn_item_started(turn_context, &compaction_item)
        .await;
    sess.start_new_context_window(&step_context, world_state, preserved_turn_items)
        .await;
