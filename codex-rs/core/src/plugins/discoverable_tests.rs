...
fn plugins_manager_for_config(config: &crate::config::Config) -> PluginsManager {
    let skills_service = Arc::new(crate::HostSkillsService::new(
        config.codex_home.clone(),
        /*bundled_skills_enabled*/ false,
    ));
    PluginsManager::new(config.codex_home.to_path_buf(), skills_service)
}
