...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Better Codex and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Better Codex and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of Better Codex."
        )),
        "{:?}",
        config.startup_warnings
    );
...
-> anyhow::Result<()> {
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = crate::plugins::plugins_manager_for_config(&config);
...
"#,
    ...
    assert_eq!(
        err.to_string(),
        "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; Better Codex would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode."
    );
    Ok(())
}
...
#[tokio::test]
async fn feature_requirements_cannot_enable_guardian_v2_authority() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
...
"#,
    ...
    assert!(!config.features.enabled(Feature::GuardianV2));
    assert!(
        config
            .startup_warnings
            .iter()
            .any(|warning| warning.contains("Ignoring unavailable `features` requirement"))
    );
    Ok(())
}
...
#[tokio::test]
async fn guardian_v2_authority_cannot_be_enabled_from_config() -> std::io::Result<()> {
    let codex_home = TempDir::new()?;
    std::fs::write(
        codex_home.path().join(CONFIG_TOML_FILE),
        r#"[features]
