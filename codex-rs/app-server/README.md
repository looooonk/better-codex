...
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values stored in `config.toml`. The response always includes `layers` (`null` unless `includeLayers` is true), and every returned layer includes `disabledReason` as a string or `null`.
