...
`better-codex app-server` is the machine-readable interface Better Codex exposes to rich clients.
...
Similar to [MCP](https://modelcontextprotocol.io/), `better-codex app-server` supports bidirectional communication using JSON-RPC 2.0 messages (with the `"jsonrpc":"2.0"` header omitted on the wire).
...
app-server process instead of starting the local process host. The built-in loopback TCP host
prints its HTTP endpoint on the first stdout line and a server-issued capability on the second. Put that capability in a
private environment variable and name it with `--code-mode-host-token-env ENV`; the capability is
never accepted in the URL or command line. Plaintext `http://` endpoints must use a literal
loopback IP address and require this capability. Endpoint-only `https://` remains available for
trusted external deployments, which are responsible for authenticating their own transport;
`--code-mode-host-token-env` may also be used with HTTPS. `unix:` endpoints must name an absolute
local socket path. HTTP URLs must not include credentials, a path, a query, or a fragment. Remote
host selection requires the `code_mode_host` feature and is independent of the app-server
`--listen` transport.

For example, capture the two output lines from
`codex-code-mode-host --listen grpc://127.0.0.1:0`, store the second as `CODE_MODE_HOST_TOKEN`, and
start the app server with `--code-mode-host "$endpoint"` and
`--code-mode-host-token-env CODE_MODE_HOST_TOKEN`. Do not write the capability to logs or shell
history.
...
Currently, you can dump a TypeScript version of the schema using `better-codex app-server generate-ts`, or a JSON Schema bundle via `better-codex app-server generate-json-schema`. Each output is specific to the version of Better Codex you used to run the command, so the generated artifacts are guaranteed to match that version.

```
better-codex app-server generate-ts --out DIR
better-codex app-server generate-json-schema --out DIR
```
...
Clients must send a single `initialize` request per transport connection before invoking any other method on that connection, then acknowledge with an `initialized` notification. The server returns the user agent string it will present to upstream services, `codexHome` for the server's `CODEX_HOME` directory, and `platformFamily` and `platformOs` strings describing the app-server runtime target; subsequent requests issued before initialization receive a `"Not initialized"` error, and repeated `initialize` calls on the same connection receive an `"Already initialized"` error.
...
Applications building on top of `better-codex app-server` should identify themselves via the `clientInfo` parameter.
...
- Experimental `runtimeWorkspaceRoots` request fields accept at most 64 paths, each at most 16,384 bytes in its native encoded form.
...
- `thread/queue/list` — experimental; page pending submissions in dispatch order with `cursor` and `limit`. Indeterminate work remains visible here while it blocks automatic dispatch.
- `thread/queue/update` — experimental; replace the input of a pending submission. An indeterminate submission whose input is already durable cannot be updated.
- `thread/queue/delete` — experimental; delete a pending submission and return whether it was removed. Deleting the indeterminate blocking submission acknowledges it and resumes later queued work.
...
- `thread/queue/start` — experimental; explicitly start the next or selected submission on a loaded, subscribed thread. A successful durable claim, including an idempotent already-claimed selection, atomically resumes a queue paused by an interruption or token-budget abort. When recovery cannot prove that queued input was consumed, the indeterminate submission becomes the next explicit retry target; when its input is already durable, it must be deleted instead. Missing subscriptions, invalid selections, empty queues, and pre-existing active claims leave the queue and its pause intact. If admission fails after a new claim, the explicit resume remains in effect.
...
- `thread/revert` — experimental. Replace a loaded paginated thread's durable history with the prefix strictly before `beforeTurnId`, while preserving its thread id and active subscriptions. The operation interrupts an active turn, retains prior rollout bytes outside the active session namespace, atomically replaces the thread's stable legacy-compatible head, returns pagination cursors for the retained history, and emits `thread/reverted`. It does not revert local file changes.
...
- `plugin/search` — search local and remote plugin catalogs by text, with optional `scope`, `cwds`, `cursor`, and `limit`. The first page merges matching local plugins with the remote page and deduplicates shared identities while retaining local installation state; cursor pages contain only remote results. Limits are clamped to `1..=100`. When remote discovery is disabled, unscoped and workspace searches still use the remote workspace directory; global and personal searches fall back to local catalogs. API-key sessions search local catalogs only (**experimental; do not call from production clients yet**).
...
- `mcpServer/oauth/login` — start an OAuth login for a configured MCP server; pass `threadId` to resolve servers from that thread's selected plugins and executor, and receive an `authorizationUrl` followed by `mcpServer/oauthLogin/completed` once the browser flow finishes.
...
- `feedback/upload` — submit a feedback report (classification + optional reason/logs, `threadId`, and optional `extraLogFiles` attachments array); returns the tracking thread id.
- `config/read` — fetch the effective config on disk after resolving config layering, including opaque `desktop` values stored in `config.toml`. Packaged defaults apply at the lowest precedence but are omitted from returned layer and origin metadata. The response always includes `layers` (`null` unless `includeLayers` is true), and every returned layer includes `disabledReason` as a string or `null`.
...
- `config/value/write` — write a single config key/value to the user's config.toml on disk; dotted paths such as `desktop.someKey` use the same generic write surface. An `expectedVersion` check and the write are serialized across app-server processes that share a Codex home.
- `config/batchWrite` — apply multiple config edits atomically to the user's config.toml on disk, with optional `reloadUserConfig: true` to hot-reload loaded threads, including multiple `desktop.*` edits. An `expectedVersion` check and the write are serialized across app-server processes that share a Codex home.
...
### Example: Search plugins (experimental)

The first request can include local working directories. Follow `nextCursor` without changing the
search term, scope, or limit; later pages are remote-only.

```json
{ "method": "plugin/search", "id": 9, "params": {
    "searchTerm": "calendar",
    "scope": "global",
...
{ "id": 9, "result": {
    "data": [
        {
            "plugin": { "id": "calendar@openai-curated-remote", "name": "calendar" },
...
To discard a paginated thread's selected turn and every later turn, call experimental `thread/revert`. The thread remains loaded under the same id, and retained history is available through the returned cursors. Prior rollout bytes are retained as internal revisions while the active head keeps one downgrade-compatible filename.

```json
{ "method": "thread/revert", "id": 14, "params": {
    "threadId": "thr_123",
    "beforeTurnId": "turn_456"
} }
{ "id": 14, "result": {
    "thread": { "id": "thr_123", "turns": [], … },
...
{ "method": "thread/reverted", "params": { "threadId": "thr_123" } }
```
...
- `sourceKinds` — restrict results to specific sources; omit or pass `[]` for interactive sessions only (`cli`, `vscode`, and named `custom` integrations).
...
- `fs/readFile` returns files up to 64 MiB as base64 bytes via `dataBase64`, and `fs/writeFile` always expects base64 bytes in `dataBase64`.
...
- `commandExecution` — `{id, command, cwd, status, commandActions, aggregatedOutput?, exitCode?, durationMs?}` for sandboxed commands; `status` is `inProgress`, `completed`, `failed`, or `declined`. Ordinary execution items and their replay expose `command` and `commandActions` as redacted display values, not executable commands. Read actions expose their absolute `path` using the executor's native path syntax, including when the executor and app-server run on different operating systems.
...
- **Personal access token (`personalAccessToken`)**: Codex uses a ChatGPT-backed personal access token loaded outside the app-server login RPCs, such as with `better-codex login --with-access-token` or `CODEX_ACCESS_TOKEN`.
...
- `account/usage/read` — fetch the ChatGPT account token-activity summary and daily buckets. This method retains its parameterless request and response shape.
- `account/threadUsage/read` — pass a valid `threadId` to read estimated credits, optional cost, and usage breakdowns for one thread using the active account. The `threadUsage` response is `null` when that billing route is unavailable.
...
`better-codex app-server` schema generation defaults to the stable API surface (experimental fields and methods filtered out). Pass `--experimental` to include experimental methods/fields in generated TypeScript or JSON schema:
...
better-codex app-server generate-ts --out DIR --experimental
better-codex app-server generate-json-schema --out DIR --experimental
```
