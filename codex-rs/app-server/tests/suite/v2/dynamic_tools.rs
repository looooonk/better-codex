...
async fn dynamic_tool_call_round_trip_handles_content_items() -> Result<()> {
    ...
    // Audio remains available on the app-server item, but the default mock model does not support
    // audio input, so history normalization omits it from the model-visible payload.
