...
#[test]
fn malformed_managed_requirements_do_not_fall_back_to_defaults() -> Result<()> {
    let codex_home = TempDir::new()?;
    std::fs::write(
    ...
    let stderr = String::from_utf8(output.stderr)?;
    assert!(
        stderr.contains("requirements.toml"),
        "expected managed requirements error in stderr, got: {stderr}"
    );
    assert!(
        !stderr.contains("using defaults"),
        "managed requirements must never fall back to permissive defaults"
    );
    Ok(())
}
