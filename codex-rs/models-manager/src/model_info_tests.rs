...
#[test]
fn base_instruction_override_preserves_catalog_review_and_permission_messages() {
    let mut model = model_info_from_slug("unknown-model");
    let auto_review = AutoReviewMessages {
        policy: Some("catalog policy".to_string()),
        policy_template: Some("catalog template".to_string()),
    };
