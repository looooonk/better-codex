...
pub(super) trait AppShellBackend {
    ...
    fn thread_queue_list_in_background(
        &self,
        thread_id: ThreadId,
    ) -> impl std::future::Future<Output = Result<Vec<QueuedSubmission>>> + Send + 'static;

    fn thread_queue_mutate_in_background(
        &self,
        thread_id: ThreadId,
        rpc: QueueRpc,
    ) -> impl std::future::Future<Output = Result<QueueRpcResponse>> + Send + 'static;
...
impl AppShellBackend for AppServerSession {
    ...
    fn thread_queue_list_in_background(
        ...
        super::backend_background::list_queued_messages(
            AppServerSession::request_handle(self),
            thread_id,
        )
    }
    ...
    fn thread_queue_mutate_in_background(
        ...
        super::backend_background::mutate_queue(
            AppServerSession::request_handle(self),
            thread_id,
            rpc,
        )
