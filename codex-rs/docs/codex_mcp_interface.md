...
- Server command: `better-codex mcp-server` (or the standalone development
  binary `codex-mcp-server`)
...
```bash
better-codex mcp-server | your_mcp_client
```

For a simple inspection UI, you can also try:

```bash
npx @modelcontextprotocol/inspector better-codex mcp-server
```

Use the separate `better-codex mcp` subcommand to manage configured MCP server launchers in `config.toml`.
