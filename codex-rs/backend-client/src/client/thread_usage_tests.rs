...
#[tokio::test]
async fn get_thread_usage_rejects_unbounded_inputs_without_a_request() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
    ...
    let error = client
        ...
        .await
        .expect_err("reject oversized thread ID");
...
async fn get_thread_usage_rejects_an_oversized_response_body() {
    ...
    let error = client
        .get_thread_usage("thread-123")
        .await
        .expect_err("reject oversized response body");

    assert!(error.to_string().contains("response exceeds"));
}

#[tokio::test]
async fn get_thread_usage_rejects_unbounded_breakdown_groups() {
    let server = MockServer::start().await;
    let groups = (0..=MAX_THREAD_USAGE_GROUPS)
        .map(|_| {
            json!({
                "model": "gpt-5.4",
                "reasoning_effort": "high",
    ...
    let error = client
        .get_thread_usage("thread-123")
        .await
        .expect_err("reject unbounded breakdown groups");

    assert!(error.to_string().contains("breakdown groups"));
}
