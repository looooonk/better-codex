...
cd path/to/better-codex/codex-rs
cargo build
...
A non-privileged user would then run Better Codex as follows, specifying the `model_provider` dynamically:

```shell
PROXY_PORT=$(jq .port /tmp/server-info.json)
PROXY_BASE_URL="http://127.0.0.1:${PROXY_PORT}"
better-codex exec -c "model_providers.openai-proxy={ name = 'OpenAI Proxy', base_url = '${PROXY_BASE_URL}/v1', wire_api='responses' }" \
...
- Authentication is fixed to `Authorization: Bearer <key>` to match Better Codex CLI expectations.
