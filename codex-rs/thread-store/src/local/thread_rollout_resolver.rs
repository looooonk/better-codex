...
async fn resolve(
    ...
    {
        return resolve_fallback_path(store, thread_id, path, scope).await;
    }
    ...
    .map_err(|err| lookup_error(thread_id, err))?;
    match path {
        Some(path) => resolve_fallback_path(store, thread_id, path, scope).await,
        None => Ok(None),
    }
}
...
pub(super) async fn rollout_id_for_thread_path(
    ...
    validate_thread_path(path, thread_id, history_mode).await?;
    rollout_id_from_path_or_legacy_thread_id(path, thread_id, history_mode)
}
...
async fn validate_thread_path(
    ...
    match codex_rollout::read_session_meta_line(path).await {
        ...
        Err(_) if history_mode == ThreadHistoryMode::Legacy => {
            rollout_id_from_unreadable_legacy_path(path, thread_id).map(|_| ())
        }
        Err(err) => Err(ThreadStoreError::InvalidRequest {
            message: format!("failed to read paginated rollout metadata {}: {err}", path.display()),
        }),
    }
}
...
fn validate_session_meta(
    ...
    if expected_history_mode.is_some_and(|expected| expected != actual_history_mode) {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!("rollout history mode disagrees with metadata: {}", path.display()),
        });
    }
    Ok(())
}
...
pub(super) fn rollout_id_from_path_or_legacy_thread_id(
    ...
    match codex_rollout::rollout_id_from_path(path) {
        Some(rollout_id) => Ok(rollout_id),
        None if history_mode == ThreadHistoryMode::Legacy => Ok(thread_id),
        None => Err(ThreadStoreError::InvalidRequest {
            message: format!(
