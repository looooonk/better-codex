...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `better-codex login --device-auth` instead."
    );
}

async fn login_with_chatgpt(options: ServerOptions) -> std::io::Result<()> {
...
pub async fn run_login_with_chatgpt(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    if login_policy::ensure_login_method_allowed(&auth_config, ForcedLoginMethod::Chatgpt).is_err()
    {
...
pub async fn run_login_with_api_key(
    ...
    let config = load_config_or_exit(cli_config_overrides).await;
    let auth_config = config.auth_config();
    if login_policy::ensure_login_method_allowed(&auth_config, ForcedLoginMethod::Api).is_err() {
    ...
    match login_policy::persist_api_key(&auth_config, &api_key) {
        Ok(_) => {
            eprintln!("{LOGIN_SUCCESS_MESSAGE}");
            std::process::exit(0);
        }
...
pub async fn run_login_with_access_token(
    ...
    match login_policy::persist_access_token(&auth_config, &access_token).await {
        Ok(_) => {
            eprintln!("{LOGIN_SUCCESS_MESSAGE}");
            std::process::exit(0);
        }
...
pub fn read_api_key_from_stdin() -> String {
    read_stdin_secret(
        "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | better-codex login --with-api-key`.",
...
pub fn read_access_token_from_stdin() -> String {
    read_stdin_secret(
        "--with-access-token expects the access token on stdin. Try piping it, e.g. `printenv CODEX_ACCESS_TOKEN | better-codex login --with-access-token`.",
...
pub async fn run_login_with_device_code(
    ...
    match login_policy::run_managed_device_code_login(
        &auth_config,
        issuer_base_url,
...
pub async fn run_login_with_device_code_fallback_to_browser(
    ...
    let mut opts = match login_policy::chatgpt_server_options(
        &auth_config,
        client_id.unwrap_or(CLIENT_ID.to_string()),
    )
    .await
