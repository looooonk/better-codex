            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation(
                            "Fix the reported config error, then rerun better-codex doctor.",
                        )
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
    ...
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH better-codex #{}: {path}", index + 1)),
        );
    }
...
fn codex_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["better-codex"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "better-codex"]);
...
fn auth_check(config: &Config) -> DoctorCheck {
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check.remediation(
                    "Run better-codex login again or provide a supported auth env var.",
                );
            }
            check
        }
        ...
        .details(details)
        .remediation(
            "Run better-codex login or provide an API key through a supported auth env var.",
        ),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run better-codex login again."),
    }
}
...
mod tests {
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running Better Codex."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running Better Codex.".to_string())
        );
    }
