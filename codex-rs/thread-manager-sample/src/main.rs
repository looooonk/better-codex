...
use codex_core_api::TuiAppTheme;
...
#[command(
    ...
    about = "Run one Better Codex turn through ThreadManager and print mapped notifications as newline-delimited JSON."
)]
struct Args {
...
async fn run_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let NewThread {
        ...
        .start_thread(config)
        .await
        .context("start Better Codex thread")?;
    ...
    turn_output?;
    shutdown_result.context("shut down Better Codex thread")?;
...
fn new_config(model: Option<String>, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<Config> {
    ...
    let mut config = Config {
        ...
        tui_app_theme: TuiAppTheme::TokyoNight,
        tui_theme: None,
...
async fn run_turn(thread: &CodexThread, thread_id: &str, prompt: String) -> anyhow::Result<()> {
    ...
    loop {
        let event = thread
            .next_event()
            .await
            .context("read Better Codex event")?;
